...
Then produce a Mermaid flowchart that lays out colored boxes horizontally — one box per color — each labeled with the hex code and a short role (e.g., Primary, Accent, Background).

Return only two things in JSON-serializable form (the agent runtime wrapper will extract fields):
1) The Mermaid diagram code
2) A color summary as JSON mapping role -> hex and an optional short justification.
